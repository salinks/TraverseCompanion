...
class UniversityFilterAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    lateinit var mContext: Activity
    private var mList: List<UniversityFiltersItem>? = null


    fun setData(activity: Activity, data: List<UniversityFiltersItem>?) {
        mList = data
        mContext = activity
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (getItemViewType(position) == VIEW_TYPES.MainView) {
            ...
            mHolder.binding?.tvFilterName?.text = mData?.uName
