...
class AmenitiesAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    lateinit var mContext: Activity
    private var mList: List<AmenitiesItem>? = null


    fun setData(activity: Activity, data: List<AmenitiesItem>?) {
        mList = data
        mContext = activity
    ...
    internal inner class MainViewHolder(itemView: View?) : RecyclerView.ViewHolder(itemView!!) {
        val binding: LayoutAmenityItemBinding? = DataBindingUtil.bind(itemView!!)
        ...
        return when (viewType) {
            ...
            VIEW_TYPES.MainView -> {
                view = LayoutInflater.from(parent.context)
                    .inflate(R.layout.layout_amenity_item, parent, false)
                MainViewHolder(view)
            }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (getItemViewType(position) == VIEW_TYPES.MainView) {
            ...
            molder.binding?.tvUAmenityName?.text = mData?.amenityName
