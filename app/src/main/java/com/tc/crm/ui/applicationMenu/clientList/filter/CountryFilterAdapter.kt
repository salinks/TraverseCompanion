...
class CountryFilterAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    lateinit var mContext: Activity
    private var mList: List<CountryFilterItem>? = null


    fun setData(activity: Activity, data: List<CountryFilterItem>?) {
        mList = data
        mContext = activity
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (getItemViewType(position) == VIEW_TYPES.MainView) {
            ...
            mHolder.binding?.tvFilterName?.text = mData?.cName
