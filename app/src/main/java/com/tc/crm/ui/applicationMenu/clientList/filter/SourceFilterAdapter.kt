...
class SourceFilterAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    lateinit var mContext: Activity
    private var mList: List<SourceFilterItem>? = null


    fun setData(activity: Activity, data: List<SourceFilterItem>?) {
        mList = data
        mContext = activity
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (getItemViewType(position) == VIEW_TYPES.MainView) {
            ...
            mHolder.binding?.tvFilterName?.text = mData?.sName
