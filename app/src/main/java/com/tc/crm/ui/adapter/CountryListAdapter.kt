...
import com.tc.crm.R
import com.tc.crm.databinding.LayoutCountryListItemBinding
...
class CountryListAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    private var mList: List<CountryListItem>? = null


    fun setData(activity: Activity, data: List<CountryListItem>?) {
        mList = data
        mContext = activity
    ...
    internal inner class MainViewHolder(itemView: View?) : RecyclerView.ViewHolder(itemView!!) {
        val binding: LayoutCountryListItemBinding? = DataBindingUtil.bind(itemView!!)
        ...
        return when (viewType) {
            ...
            VIEW_TYPES.MainView -> {
                view = LayoutInflater.from(parent.context)
                    .inflate(R.layout.layout_country_list_item, parent, false)
                MainViewHolder(view)
            }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (getItemViewType(position) == VIEW_TYPES.MainView) {
            ...
            molder.binding?.tvCountryName?.text = mData?.nicename
            molder.binding?.tvCountryCode?.text = mData?.iso
            molder.binding?.tvPhoneCode?.text = "+"+mData?.phonecode
